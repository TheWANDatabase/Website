<script lang="ts">
  ...
  import { liveState } from '$lib/stores'

	export let data: CachedEntity<Episode>;

	export let { cached, data: episode, queryTime } = data as any;

  // We want the youtube ID from the floatplane field
  // This is done this way because we need to discriminate
  // the stream from the actual vods, and so it is copied ovar
  // so that we can filter it out from search results
  episode.id = episode.floatplane;
  episode.floatplane = "LIVE_STREAM";

	console.debug(`==== Fetch Report ====\nCached: ${cached}\nQuery Time: ${queryTime}ms`);
  console.debug("page.svelte - ", episode);
...
<div class="container">
	<div class="player">
		...
		<div class="details">
			<h1 style="grid-area:title;">{$liveState.title}</h1>
