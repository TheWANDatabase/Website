...
async function infinite() {
  ...
  try {
    ...
    f.startDate = new Date(f.startDate).toISOString()
    f.endDate = new Date(f.endDate).toISOString()
    ...
    const feed = await (await fetcher('episodes', {
      ...
      body: JSON.stringify({
        ...
        limit: 20,
        flags: {
          include: ['cast_ids', 'topic_count']
        }
      })
    })).json();
    fd.value = fd.value.concat(feed.filter(({id}) => !fdm.value.has(id)));
    episodeCount.value = fd.value.length;
    offset = fd.value.length;
  } catch (e) {
    console.error(e)
  }
  loading.value = false
...
<template>
  <div class="flex-col min-h-screen">
    <div
        :class="`shadow-sm shadow-black z-10 flex-col mt-3 mx-auto mb-2 bg-${cfg.theme.primary}-800 rounded justify-evenly max-w-fit sticky top-1`"
        style="top: 0.25rem;">
      <div class="flex p-2">
        <div class="w-42 flex-col my-auto mr-4 justify-evenly">
          <span class="flex w-56 justify-between my-1">
            <p :class="`text-${cfg.theme.primary}-200 h-5 inline align-middle`">Enable infinity scrolling</p>
            <UTooltip text="Enable Infinity Scroll">
              <UToggle v-model="allowInfinite" :color="cfg.theme.primary" on-icon="i-heroicons-check-20-solid"
                       :ui="{ inactive: `bg-${cfg.theme.primary}-200 dark:bg-${cfg.theme.primary}-700` }"
                       off-icon="i-heroicons-x-mark-20-solid"/>
            </UTooltip>
          </span>
          <span class="flex w-56 justify-between my-1">
            <p :class="`text-${cfg.theme.primary}-200 h-5 inline align-middle`">Hide Content Warnings</p>
          ...
          <span class="flex w-56 justify-between my-1">
            <p :class="`text-${cfg.theme.primary}-200 h-5 inline align-middle`">Hide Corrupted Content</p>
        ...
        <div class="flex-col mb-auto mt-0">
          <div class="flex max-w-fit mt-3 mx-auto mb-2 p-2 justify-evenly">
            ...
            <USelectMenu v-model="filters.members" multiple :options="cst" placeholder="Select Cast Members" searchable
                         :color="cfg.theme.primary" :ui="{width: 'w-72'}" :ui-menu="{width: 'w-72'}"
            ...
            <UButton class="ml-10" label="Apply Filters" :loading="loading" icon="i-heroicons-magnifying-glass-20-solid"
                     :color="cfg.theme.primary" variant="soft" @click="filter"/>
          </div>
